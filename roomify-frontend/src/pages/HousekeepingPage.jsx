...
const HousekeepingPage = () => {
  ...
  const fetchRooms = async () => {
    try {
      ...
      // Housekeeping focuses on rooms that are Occupied or under Maintenance
  ...
  const handleUpdateStatus = async (roomId, newStatus) => {
    ...
    if (!window.confirm(`Set Room ${room.roomNumber} status to '${statusToSet}'?`))
      return;

    try {
      setStatusUpdating(roomId);
      await axios.put(
        `${API_BASE_URL}/api/housekeeping/rooms/${roomId}/status`,
        { status: statusToSet },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      setRooms((prevRooms) =>
        prevRooms.map((r) =>
          r._id === roomId
            ? { ...r, status: statusToSet, pendingStatus: undefined }
            : r
        )
      );
    } catch (err) {
      ...
      {rooms.length === 0 ? (
        <Card className="dark:bg-gray-700 dark:border-gray-600">
          <CardContent className="p-6 text-center text-gray-500 dark:text-gray-400">
            No rooms currently require service.
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {rooms.map((room) => (
            <Card
              key={room._id}
              className={`shadow-md dark:bg-gray-700 ${
            ...
            >
              ...
              <CardContent className="pt-4 space-y-4">
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300">
                    Price: ${room.pricePerNight?.toFixed(2)}
                  </p>
                </div>

                <div className="space-y-2">
                  <Label
                    htmlFor={`status-${room._id}`}
                  ...
                  <Select
                    ...
                    onValueChange={(newStatus) =>
                      handleSelectChange(room._id, newStatus)
                    }
                    disabled={statusUpdating === room._id}
                  >
                    <SelectTrigger
                      id={`status-${room._id}`}
                    ...
                    <SelectContent>
                      <SelectItem value="Available" className="text-green-600">
                        Available (Clean)
                      </SelectItem>
                      <SelectItem
                        value="Maintenance"
                ...
                <Button
                  onClick={() => handleUpdateStatus(room._id, room.pendingStatus)}
                  disabled={
                    statusUpdating === room._id ||
                    !room.pendingStatus ||
                    room.pendingStatus === room.status
                  }
                  className="w-full"
                >
                  {statusUpdating === room._id ? (
