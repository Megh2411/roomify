...
const GuestBookingPage = () => {
    ...
    const fetchRooms = async () => {
        ...
        try {
            ...
            // Fetch ALL rooms. Availability is checked on the POST /api/bookings endpoint.
            const { data: roomData } = await axios.get('http://localhost:5000/api/rooms', {
                headers: { Authorization: `Bearer ${token}` }
            });
            setRooms(roomData.filter(r => r.status === 'Available')); // Only show available rooms for booking
        } catch (err) {
    ...
    // --- Room Search/Filter Logic (Client-side validation) ---
    const filterRooms = () => {
        ...
        setError(null);
        // In a complex application, this would trigger a dedicated API search.
        // For simplicity, we assume the user is browsing the inventory.
        alert(`Simulating search for ${days} nights from ${checkInDate}. Final availability checked on reservation.`);
    };
    ...
    const handleReserve = async (roomId) => {
        ...
        try {
            ...
            const payload = {
                roomIds: [roomId], // Booking one room
                checkInDate,
                checkOutDate,
                // Services can be added later
            };

            // Call the POST /api/bookings endpoint
            ...
            // Redirect guest to a confirmation or my-bookings page
            navigate('/book'); 
    ...
    if (loading) return <div className="p-8 text-center"><Loader2 className="h-6 w-6 animate-spin mx-auto mb-4" /> Loading rooms...</div>;
    
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-800 p-6 sm:p-10">
            <header className="text-center mb-10">
                <h1 className="text-4xl font-extrabold text-blue-900 dark:text-blue-400">Welcome, {userInfo?.name || 'Guest'}!</h1>
                <p className="text-gray-600 dark:text-gray-400 mt-2">Find your perfect stay at Roomify based on your travel dates.</p>
            </header>
            ...
            {/* Error Message */}
            {error && <div className="max-w-4xl mx-auto mb-6 p-4 bg-red-100 dark:bg-red-900 dark:text-red-300 text-red-700 rounded-lg">{error}</div>}


            {/* Room Grid */}
            <div className="max-w-6xl mx-auto">
                <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-gray-200">Available Rooms ({rooms.length})</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {rooms.map((room) => (
                        <Card key={room._id} className="overflow-hidden shadow-xl hover:shadow-2xl transition-shadow duration-300 dark:bg-gray-700 dark:border-gray-600">
                            {/* --- UPDATED AESTHETICS: Softer Placeholder Image --- */}
                            <img 
                            ...
                            <CardContent className="px-4 pb-4 pt-0">
                                <p className="text-lg font-semibold flex items-center mb-3 dark:text-white">
                                    ...
                                    ${room.pricePerNight.toFixed(2)} <span className="text-gray-500 dark:text-gray-400 font-normal text-sm ml-1">/ night</span>
                                </p>
                                <Button 
                ...
                {rooms.length === 0 && !loading && (
                     <div className="text-center p-10 text-gray-500 dark:text-gray-400 border rounded-lg dark:border-gray-600">No rooms currently available based on inventory.</div>
                )}
            </div>
        </div>
    );
